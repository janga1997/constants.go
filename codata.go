...
type Constant struct {
	value, uncertainity float64
	units               string
}

func parse_constants(d string) map[string]Constant {
	constants := make(map[string]Constant)
	lines := strings.Split(d, "\n")
	for i := range lines {
        length := len(lines[i])
		name := strings.Trim(lines[i][:55], " ")
		val_string := strings.Replace(lines[i][55:77], " ", "", -1)
		val_string = strings.Replace(val_string, "...", "", -1)
		val, _ := strconv.ParseFloat(val_string, 64)
		uncert_string := strings.Replace(lines[i][77: int(math.Min(99, float64(length)))], " ", "", -1)
		uncert_string = strings.Replace(uncert_string, "(exact)", "0", -1)
		uncert, _ := strconv.ParseFloat(uncert_string, 64)
		units := strings.Trim(lines[i][int(math.Min(99, float64(length))):], " ")
		constants[name] = Constant{val, uncert, units}
	}
	return constants
}

// var Physical_constants = make(map[string]Constant)

var _Physical_constants_2002 = parse_constants(txt2002)
var _Physical_constants_2006 = parse_constants(txt2006)
var _Physical_constants_2010 = parse_constants(txt2010)
var _Physical_constants_2014 = parse_constants(txt2014)

var all_constants = [4]map[string]Constant{_Physical_constants_2002, _Physical_constants_2006, _Physical_constants_2010, _Physical_constants_2014}

func map_update(array [4]map[string]Constant) map[string]Constant{
    
	var dummy_constants = make(map[string]Constant)

    for _, element := range all_constants {
        for k := range element {
            dummy_constants[k] = element[k]
        }
    }

    return dummy_constants
}

var Physical_constants = map_update(all_constants)

func Value(key string) float64{
    return Physical_constants[key].value
}

func Units(key string) string{
    return Physical_constants[key].units
}

func Precision(key string) float64{
